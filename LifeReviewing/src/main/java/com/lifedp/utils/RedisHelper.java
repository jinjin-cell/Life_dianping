...
public class RedisHelper {
     ...
     * 使用 Spring Data Redis StreamOperations，兼容所有 Redis 5.0+ 版本。
     * 如果组已存在 (BUSYGROUP) 则正常返回。
     */
    public String createConsumerGroup(String streamKey, String groupName) {
        try {
            // 使用原生 XGROUP CREATE 命令（兼容 Spring Data Redis 各版本）
            ...
            // Spring 可能把 BUSYGROUP 包装成 RedisSystemException("Error in execution")
    ...
    public List<Map<String, String>> readFromStream(String streamKey, String groupName,
        ...
        LettuceConnectionFactory factory =
                (LettuceConnectionFactory) streamStringRedisTemplate.getConnectionFactory();
        org.springframework.data.redis.connection.lettuce.LettuceConnection conn =
     ...
     * 返回格式与之前兼容: {recordId, consumer, idleMs, deliveryCount}
     */
    public List<Map<String, Object>> pendingStream(String streamKey, String groupName,
        ...
        try {
            ...
            for (io.lettuce.core.models.stream.PendingMessage pm : pending) {
                ...
                map.put("recordId", pm.getId()); // getId() always returns String
                map.put("consumer", pm.getConsumer()); // PendingMessage consumer 固定返回 String
    ...
    public org.springframework.data.geo.GeoResults<org.springframework.data.redis.connection.RedisGeoCommands.GeoLocation<String>> geoSearch(
        ...
        org.springframework.data.redis.domain.geo.GeoReference<String> geoRef =
                org.springframework.data.redis.domain.geo.GeoReference.fromCircle(circle);
        org.springframework.data.redis.core.GeoOperations<String, String> geoOps = stringRedisTemplate.opsForGeo();
        org.springframework.data.geo.GeoResults<org.springframework.data.redis.connection.RedisGeoCommands.GeoLocation<String>> results =
                geoOps.search(key, geoRef, radius);
        if (results != null && results.getContent().size() > limit) {
            List<org.springframework.data.geo.GeoResult<org.springframework.data.redis.connection.RedisGeoCommands.GeoLocation<String>>> limited =
                    results.getContent().subList(0, (int) limit);
            return new org.springframework.data.geo.GeoResults<>(limited, results.getAverageDistance());
        }
        return results;
    }
}
